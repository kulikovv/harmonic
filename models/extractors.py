...
def squeezenet(pretrained=True, path=None):
    if pretrained:
        if path is not None:
            model = SqueezeNet(False)
            load_weights_sequential(model, torch.load(path))
            return model
    return SqueezeNet(pretrained)


def densenet(pretrained=True, path=None):
    if pretrained:
        if path is not None:
            model = SqueezeNet(False)
            load_weights_sequential(model, torch.load(path))
            return model
    return DenseNet(pretrained)

def resnet18(pretrained=True, path=None):
...
def resnet34(pretrained=True, path=None):
...
def resnet50(pretrained=True, path=None):
...
def resnet101(pretrained=True, path=None):
    model = ResNet(Bottleneck, [3, 4, 23, 3])
    if pretrained:
        if path is None:
            load_weights_sequential(model, model_zoo.load_url(model_urls['resnet101']))
        else:
            load_weights_sequential(model, model_zoo.load_url(path))
    return model


def resnet152(pretrained=True, path=None):
